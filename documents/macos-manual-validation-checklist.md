...
Task 6.3 scope: validate the macOS end-to-end flow with the current MVP scaffold.
...
- [ ] Launch the app on macOS and confirm the main window opens without crashes.
- [ ] Confirm the startup state shows the expected environment banner or ready state.
- [ ] Select files with the native macOS file picker and confirm the chosen paths appear in the UI.
...
- [ ] Open the output folder from the results panel and confirm Finder opens the expected directory.
- [ ] Quit the app by closing the last window and confirm macOS app behavior matches the Electron default for the platform.
- [ ] Reopen the app from the Dock and confirm the window can be restored without stale job state in the UI.

## Failure And Gating Checks

Task 6.5 scope: validate the blocked and degraded cases on macOS.
...
- Finder opens the intended output directory
